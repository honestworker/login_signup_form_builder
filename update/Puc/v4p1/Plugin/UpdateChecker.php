		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
				...
				if ( $status == 'no_update' ) {
					...
					if (!empty(get_option('dig_purchasecode'))) {
						$message = sprintf(_x('A new version of the %s plugin is available.', 'the plugin title', 'plugin-update-checker'), $title);
					}else{
						$url = get_admin_url(null,"/options-general.php?page=digits_settings&tab=activate");
						$message = sprintf(_x('A new version of the %s plugin is available. Please enter purchase code <a href="%s">here</a> to enable automatic updates.', 'the plugin title', 'plugin-update-checker'), $title,$url);
					}
