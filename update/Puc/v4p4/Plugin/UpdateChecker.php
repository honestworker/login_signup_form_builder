		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
				...
				if ( $status == 'no_update' ) {
                    ...
                    $digpc = get_option('dig_purchasecode');
                    if(empty($digpc)) {
                        $url = get_admin_url(null, "/options-general.php?page=digits_settings&tab=activate");
                        $message = sprintf(_x('A new version of the %s plugin is available. Please enter purchase code <a href="%s">here</a> to enable automatic updates.', 'the plugin title', 'digits'), $title, $url);
                    }else {
